...
export class ListTablesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private _edit(data: any): void {
    ...
    this._tableSvc.subscription.push(this._tableSvc.getTable(id).subscribe({
      next: response => {
    ...
    this._sweetAlertSvc.question('Are you sure you want to delete selected Table?').then(res => {
      if (res === ActionClicked.Yes) {
        ...
        this._tableSvc.subscription.push(this._tableSvc.delete(IDsToRemove).subscribe({
          next: response => {
            this._msgSvc.add({ severity: 'success', summary: 'Successfully', detail: 'The Table(s) was(were) deleted successfully.' })
          },
          error: err => {
            this._sweetAlertSvc.error(err);
          }
