...
export class TablesFormComponent implements OnInit {
  ...
  private async _getPayments(): Promise<void> {
    try {
      return new Promise(resolve =>
        this._tablesSvc.subscription.push(
          this._paymentsSvc.getAll().subscribe({
            next: result => {
              this.payments = cloneDeep(result.getPayments);
              this.paymentsValues = this.payments
                .filter(p => p.IdPayInstr === EPaymentInstrument.CHIPS)
                .map(c => {
                  return {
                    value: c.IdPayment,
                    label: c.PaymentName,
  ...
  private _loadTablesInitValues(): void {
    ...
    this._tablesSvc.getTableInitValues().subscribe(res => {
      this.initialValues = cloneDeep(
        res.getTableInitValues.map(({ __typename, ...rest }) => {
          return rest;
        })
      );
    });
  ...
  private _save(): void {
    ...
    this._tablesSvc.subscription.push(
      this._tablesSvc.save(this.initialValues).subscribe({
        next: response => {
          let txtMessage;
