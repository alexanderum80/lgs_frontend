...
export class CitiesService {
  ...
  getCitiesByIdCountry(idCountry: number): Observable<CitiesQueryResponse> {
    return new Observable<CitiesQueryResponse>((subscriber) => {
      this._apolloSvc
        .query<CitiesQueryResponse>(citiesApi.byCountry, { idCountry })
        .subscribe({
          next: (response) => {
            subscriber.next(response);
          },
  ...
  getCity(id: number): Observable<CitiesQueryResponse> {
    return new Observable<CitiesQueryResponse>((subscriber) => {
      this._apolloSvc
        .query<CitiesQueryResponse>(citiesApi.byId, { id })
        .subscribe({
          next: (response) => {
            subscriber.next(response);
          },
  ...
  save(): Observable<CitiesMutationResponse> {
    ...
    const cityMutation =
      payload.IdCity === 0 ? citiesApi.create : citiesApi.update;

    return new Observable<CitiesMutationResponse>((subscriber) => {
      this.subscription.push(
        this._apolloSvc
          .mutation<CitiesMutationResponse>(
            cityMutation,
            { cityInput: payload },
            ['GetCities']
          )
  ...
  delete(IDs: number[]): Observable<CitiesMutationResponse> {
    return new Observable<CitiesMutationResponse>((subscriber) => {
      this.subscription.push(
        this._apolloSvc
          .mutation<CitiesMutationResponse>(citiesApi.delete, { IDs }, [
            'GetCities',
          ])
          .subscribe({
            next: (result) => {
