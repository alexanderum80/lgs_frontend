  ...
  private _getOperationDetails(): void {
    try {
      ...
      this._operationSvc.subscription.push(
        this._operationSvc.getOperationDetails(id).subscribe({
          next: result => {
            const op = cloneDeep(
  ...
  private _save(): void {
    ...
    if (this.canDeliver) {
      this.operationDelivered.map(r => {
        totalDelivered +=
          r.IdInstrument !== EPaymentInstrument.BONUS
            ? r.Denomination! * r.Qty * r.Rate
            : 0;
    ...
    this._operationSvc.subscription.push(
      this._operationSvc.saveOperation(operationR, operationD).subscribe({
        next: response => {
          let txtAction;
