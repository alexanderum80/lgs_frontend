...
export class OperationService {
  ...
  getTodayOperation(
    ...
    return new Observable<OperationQueryResponse>(subscriber => {
      this.subscription.push(
        this._apolloSvc
          .watchQuery<OperationQueryResponse>(operationApi.today, {
            idState: operation,
          })
          .subscribe({
            next: result => {
  ...
  getOperationDetails(id: number): Observable<OperationQueryResponse> {
    return new Observable<OperationQueryResponse>(subscriber => {
      this.subscription.push(
        this._apolloSvc
          .query<OperationQueryResponse>(operationApi.detail, { id })
          .subscribe({
            next: result => {
              subscriber.next(result);
            },
  ...
  getOperation(id: number): Observable<OperationQueryResponse> {
    return new Observable<OperationQueryResponse>(subscriber => {
      this.subscription.push(
        this._apolloSvc
          .query<OperationQueryResponse>(operationApi.byId, { id })
          .subscribe({
            next: result => {
              subscriber.next(result);
            },
  ...
  saveOperation(
    ...
    const operationMutation =
      operationR.IdOperation === 0 ? operationApi.create : operationApi.update;
  ...
  deleteOperation(IDs: number[]): Observable<OperationMutationResponse> {
    return new Observable<OperationMutationResponse>(subscriber => {
      this.subscription.push(
        this._apolloSvc
          .mutation<OperationMutationResponse>(operationApi.delete, { IDs }, [
            'GetOperationsToday',
          ])
          .subscribe({
            next: result => {
  ...
  finishInitialization(): Observable<OperationMutationResponse> {
    return new Observable<OperationMutationResponse>(subscriber => {
      this.subscription.push(
        this._apolloSvc
          .mutation<OperationMutationResponse>(operationApi.finishInit, null, [
            'GetOperationsToday',
          ])
          .subscribe({
            next: result => {
  ...
  finishClosing(): Observable<OperationMutationResponse> {
    return new Observable<OperationMutationResponse>(subscriber => {
      this.subscription.push(
        this._apolloSvc
          .mutation<OperationMutationResponse>(operationApi.finishClose, null, [
            'GetOperationsToday',
          ])
          .subscribe({
            next: result => {
