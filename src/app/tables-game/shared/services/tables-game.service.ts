...
import {
  TablesGameQueryResponse,
  TablesGameMutationResponse,
} from './../models/tables-game.model';
...
export class TablesGameService {
  ...
  getTablesGame(): Observable<TablesGameQueryResponse> {
    return new Observable<TablesGameQueryResponse>((subscriber) => {
      this._apolloSvc
        .watchQuery<TablesGameQueryResponse>(tablesGameApi.all)
        .subscribe({
          next: (response) => {
  ...
  getTableGame(id: number): Observable<TablesGameQueryResponse> {
    return new Observable<TablesGameQueryResponse>((subscriber) => {
      this._apolloSvc
        .query<TablesGameQueryResponse>(tablesGameApi.byId, { id })
        .subscribe({
          next: (response) => {
            subscriber.next(response);
          },
  ...
  save(): Observable<TablesGameMutationResponse> {
    ...
    const countryMutation =
      payload.IdGame === 0 ? tablesGameApi.create : tablesGameApi.update;

    return new Observable<TablesGameMutationResponse>((subscriber) => {
      this.subscription.push(
        this._apolloSvc
          .mutation<TablesGameMutationResponse>(
            countryMutation,
            { tableGameInput: payload },
            ['GetTablesGame']
          )
          .subscribe({
            next: (result) => {
  ...
  delete(IDs: number[]): Observable<TablesGameMutationResponse> {
    return new Observable<TablesGameMutationResponse>((subscriber) => {
      this.subscription.push(
        this._apolloSvc
          .mutation<TablesGameMutationResponse>(tablesGameApi.delete, { IDs }, [
            'GetTablesGame',
          ])
          .subscribe({
            next: (result) => {
